jQuery(document).ready(function($){
	...
	$('#set-engine').submit(function () {
		...
		var presetstr = '?preset=1&c='+$('#columns', this).val()+'&u='+$('#units', this).val();
	...
	function build_presets() {
		...
		controls += '</select>';
		controls += '<label for="control-presets-select">&nbsp;&lt;- This is a good place to start</label>';
		controls += '</form>';
		
		$('#set-engine').after(controls);
	...
	function carry_flags() {
		...
		$('.carry').each(function (index) {
			// not sure why this is sometimes picking up more .carry elements but this fixes it
			if (index < units-1) {
